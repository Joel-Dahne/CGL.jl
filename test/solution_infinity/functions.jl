@testset "functions" begin
    ...
    @testset "P" begin
        ...
        @test P_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> P(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-12

        @test P_dξ_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> P_dξ(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-12

        @test P_dξ_dξ_dϵ(ξ, (λ, κ)) ≈
              fdm(ϵ -> P_dξ_dξ(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol = 1e-12
    end
    ...
    @testset "E" begin
        ...
        @test E_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> E(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-10

        @test E_dξ_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> E_dξ(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-10
    end
    ...
    @testset "B_W" begin
        ...
        @test CGL.B_W_dϵ(κ, λ) ≈ fdm(ϵ -> CGL.B_W(κF64, CGLParams(λF64; ϵ)), λF64.ϵ) rtol =
            1e-12
    end
    ...
    @testset "J_P" begin
        ...
        @test J_P_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> J_P(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-10
    end
    ...
    @testset "J_E" begin
        ...
        @test J_E_dϵ(ξ, (λ, κ)) ≈ fdm(ϵ -> J_E(ξF64, (CGLParams(λF64; ϵ), κF64)), λF64.ϵ) rtol =
            1e-10
    end
    ...
    @testset "H" begin
        @test Arblib.overlaps(
            H(ξ, (λ, κ)),
            -c_dϵ * CGL.B_W(κ, λ) * P(ξ, (λ, κ)) +
            CGL.B_W_dϵ(κ, λ) * P(ξ, (λ, κ)) * ξ^-2 +
            CGL.B_W(κ, λ) * P_dϵ(ξ, (λ, κ)) * ξ^-2,
        )

        @test H_dξ(ξ, (λ, κ)) ≈ fdm(ξ -> H(ξ, (λF64, κF64)), ξF64) rtol = 1e-12

        @test H_dξ_dξ(ξ, (λ, κ)) ≈ fdm(ξ -> H_dξ(ξ, (λF64, κF64)), ξF64) rtol = 1e-12
        @test H_dξ_dξ(ξ, (λ, κ)) ≈ fdm2(ξ -> H(ξ, (λF64, κF64)), ξF64) rtol = 1e-8
    end
end
